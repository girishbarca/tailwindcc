...
if (process.cwd() !== expectedPath) {
  console.log(
    `[!] The 'warcannon' command can only be used from ` +
      `${expectedPath}`.blue
  );
  process.exit(1);
}
...
(async () => {
  ...
  yargs
      ...
      (yargs) => {
        return yargs
          .option('skipInit', {
            ...
            type: 'boolean',
            description:
              'Skip the Terraform Init phase. Useful for development.',
          })
          .option('autoApprove', {
            alias: 'y',
            type: 'boolean',
            description:
              'Auto-approve Terraform changes. Useful for development.',
          });
      },
      async (argv) => {
        showBanner();
    ...
    .command(
      'status',
      'Shows the status of the WARCannon deployment',
      (yargs) => yargs,
      async (argv) => {
        await showStatus();
      }
    ...
    .command(
      'emptyQueue',
      'Empties the WARCannon work queue',
      (yargs) => yargs,
      async (argv) => {
    ...
    .command(
      'list [search]',
      'Shows Crawls matching the provided search string',
      (yargs) => {
    ...
    .command(
      'populate <crawl> [chunk_size] [num_chunks]',
      'Populates a job from the provided Crawl, optionally segmenting it by chunk size and count.',
      (yargs) => {
        return yargs
          .option('crawl', {
            type: 'string',
      ...
      async (argv) => {
        ...
        if (matchingCrawls.length != 1) {
          console.log(
            `[!] Provided crawl matched [${matchingCrawls.length}] Crawls. Should match exactly 1.`
              .red
          );
          return false;
        }
        ...
        try {
          ...
          console.log(
            '[!] Unable to locate the index file for this crawl.'.red
          );
          console.log(
            '[!] This might mean that the crawl is still in progress, or'.red
          );
          console.log(
            '[!] is otherwise not ready for use. Try a different crawl.'.red
          );
          return false;
        }
    ...
    .command(
      'createAthena',
      'Creates and initializes tables for Athena queries',
      (yargs) => yargs,
      async (argv) => {
    ...
    .command(
      'populateAthena <executionId>',
      'Populates a job based on a previous Athena query.',
      (yargs) => yargs,
      async (argv) => {
        console.log('[*] Loading job. Please wait...'.blue);
    ...
    .command(
      'syncResults',
      'Download results from WARCannon to the local machine',
      (yargs) => yargs,
      async (argv) => {
    ...
    .command(
      'terminate',
      'Terminate all WARCannon spot fleets',
      (yargs) => yargs,
      async (argv) => {
        ...
        const warcannonSfrs = sfrs.SpotFleetRequestConfigs.map((e) => {
          if (e.SpotFleetRequestState == 'active') {
            if (
              e.Tags.filter((t) => t.Key == 'Name' && t.Value == 'Warcannon')
                .length > 0
            ) {
        ...
        console.log(
          `[+] Sent termination request for [${warcannonSfrs.length}] Spot Fleet(s)`
            .green
        );
    ...
    .command(
      'testLocal [warcPath]',
      'Test the filter function locally',
      (yargs) => {
        return yargs
          .option('warcPath', {
            alias: 'w',
            default:
              'crawl-data/CC-MAIN-2023-40/segments/1695233505362.29/warc/CC-MAIN-20230921073711-20230921103711-00000.warc.gz',
            type: 'string',
          ...
          .option('force', {
            ...
            type: 'boolean',
            description:
              'Perform the test, ignoring changes to settings.json or matches.js.',
          })
          .option('stream', {
            alias: 's',
            default: false,
      ...
      async (argv) => {
        ...
        if (!argv.stream && !fs.existsSync('/tmp/warcannon.testLocal')) {
          console.log(
            '[!] Downloading ~1.2GiB WARC. This may take several minutes.'
          );
          await downloadS3File(
            'commoncrawl',
            argv.warcPath,
            '/tmp/warcannon.testLocal'
          );
        }
        ...
        const localTest = require(path.join(
          process.cwd(),
          '/lambda_functions/warcannon/main.js'
        ));

        const waitForLocalTest = new Promise((success, failure) => {
          localTest.main(
            { warc: argv.warcPath, stream: argv.stream },
            {},
            console.log
          );
        });
        ...
        if (!fs.existsSync(resultFile)) {
          console.log(
            `[+] Local test succeeded. Results are stored in ${path.relative(
              resultsPath,
              resultFile
            )}`
          );
    ...
    .command(
      'test [warcPath]',
      'Test the filter function in Lambda',
      (yargs) => {
        return yargs
          .option('warcPath', {
            alias: 'w',
      ...
      async (argv) => {
        ...
        console.log(
          '[*] Starting Lambda test. This may take several minutes, please be patient...'
            .blue
        );
        ...
        try {
          ...
          console.log(
            `[!] If the function timed out, your regex pattern is likely too expensive.`
          );
          console.log(
            `[!] Consider optimizing your regex, otherwise stick to local tests or limited test campaigns.`
          );
          return false;
        }
    ...
    .command(
      ...
      async (argv) => {
        ...
        console.log(
          `[+] This will request [ ${
            settings.nodeCapacity.toString().green
          } ] nodes of type [ ${settings.nodeInstanceType.join(', ').green} ]`
        );
        console.log(
          '	lasting for [ ' +
            `${(
              Math.round(settings.nodeMaxDuration / 360) / 10
            ).toString()} hours`.green +
            ' ]\n'
        );
        console.log(
          'To change this, edit your settings in ' +
            "'settings.json'".green +
            ' and re-deploy.'
        );
        ...
        const fire = await new Promise((success, failure) => {
          rl.question(
            '--> Ready to fire? [Only ' + 'Yes'.green + ' will be accepted]: ',
            function (answer) {
              rl.close();
        ...
        const sfr = await ec2
          .requestSpotFleet({
            SpotFleetRequestConfig: JSON.parse(
              fs.readFileSync('./render-warcannon/spot_request.json')
            ),
          })
          .promise();
        ...
        const url = distributions.DistributionList.Items.filter(
          (e) => e.Comment == 'Warcannon'
        )?.[0]?.DomainName;
        ...
        console.log(
          '[+] Monitor node status and progress at '.green +
            `https://${url}`.blue +
            '\n'
        );
    ...
    .command(
      'quickResults [pattern]',
      'Show the value and source of locally-synced results from files matching an optional pattern.',
      (yargs) => yargs,
      async (argv) => {
        const quickResults = fs
          .readdirSync(resultsPath)
          .filter((e) => !!!argv.pattern || e.indexOf(argv.pattern))
          .reduce((a, e) => {
            const results = JSON.parse(
              fs.readFileSync(path.join(resultsPath, e))
            );

            Object.keys(results.regex_hits).map((regex) => {
              if (!!!a[regex]) {
                a[regex] = {};
              }
              ...
              Object.keys(results.regex_hits[regex])
                ...
                .map((x) => {
                  ...
                  const firstSource = Object.keys(x).filter(
                    (k) => k != 'value'
                  )[0];
...
async function showStatus() {
  if (!fs.existsSync('render-warcannon/userdata.sh')) {
    console.log(
      `Deployed [ ${'NO'.red} ]; Run ${'warcannon deploy'.blue} to get started`
    );
    return false;
  }
  ...
  try {
    ...
    const sfr = sfrs.SpotFleetRequestConfigs.filter((e) => {
      if (e.SpotFleetRequestState == 'active') {
        return !!e.Tags.filter((t) => t.Key == 'Name' && t.Value == 'Warcannon')
          .length;
      }
    ...
    const queueStatus =
      queue.Attributes.ApproximateNumberOfMessages == 0
        ? 'EMPTY'.blue
        : `${queue.Attributes.ApproximateNumberOfMessages} Messages`.green;
    ...
    if (!sfr) {
      ...
      console.log(
        `Job Status: [ ${sfr.SpotFleetRequestState.green} ] [ ${sfr.SpotFleetRequestId.green} ]`
      );
...
async function getFullS3PrefixList(
  Bucket,
  Prefix,
...
async function getWarcannonBucket() {
  ...
  const warcannonBuckets = buckets.Buckets.map((e) => {
  ...
  if (warcannonBuckets.length > 1) {
    console.log(
      `[!] You have more than one results bucket in your account. Unable to proceed.`
    );
    return false;
  }
...
function needsRedeploy() {
  ...
  if (
    newerThan(
      'lambda_functions/warcannon/matches.js',
      'render-warcannon/backend.tf.json'
    )
  ) {
