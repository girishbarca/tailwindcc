...
exports.main = function (parser) {
  ...
  return new Promise((success, failure) => {
    process.send =
      typeof process.send == 'function' ? process.send : console.log;
    ...
    if (isLocal) {
      process.on('SIGINT', function () {
        console.log(
          `\n\n[!] Caught interrupt. Saving results to [ ` +
            `${resultsPath}localResults.json`.blue +
            ` ]\n`
        );
        ...
        console.log(
          `Average per-record Total processing time:  ${Math.round(avgTotal)}ns`
        );
        ...
        console.log(
          `Rough estimate cost for 72,000 WARC campaign: $${estimatedCost.toFixed(
            2
          )}`
        );
        console.log(
          "^ this will be wildly inaccurate (low) in 'testLocal -s' mode."
        );
    ...
    parser.on('data', (record) => {
      ...
      if (
        mime_types.length > 0 &&
        !mime_types.includes(record.warcHeader['WARC-Identified-Payload-Type'])
      ) {
        return true;
      }
      ...
      for (const r_key in regex_patterns) {
        ...
        metrics.total_hits++;
        const key = hash(domain);
        metrics.regex_hits[r_key][key] ??= { record_content };
        metrics.regex_hits[r_key][key][domain] ??= [];

        let uri = record.warcHeader['WARC-Target-URI'];
        let uris = metrics.regex_hits[r_key][key][domain];
    ...
    parser.on('end', function () {
      ...
      if (isLocal) {
        console.log(
          `\n\n[+] Parser finished. Saving results to [ ` +
            `${resultsPath}localResults.json`.blue +
            ` ]\n`
        );
        console.log('--- Performance statistics ---');
...
function toFixedLength(what, length) {
  what =
    what.length <= length ? what : what.substring(0, length - 3).concat('...');
...
if (!isLocal && !!process.argv?.[2]) {
  if (process.argv?.[2]?.indexOf('crawl-data/') !== 0) {
    console.log(
      `[!] parse_regex.js: Not isLocal, but got crawl [ ${process.argv[2]} ]`
    );
    return false;
  }
